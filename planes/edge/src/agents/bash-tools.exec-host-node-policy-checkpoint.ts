/**
 * Node-host dispatch-time policy checkpoint.
 * Re-reads the current node/caller policy immediately before a `system.run`
 * dispatch so a grant obtained earlier can never override a later deny.
 * Split out of bash-tools.exec-host-node.ts to keep that orchestrator readable.
 */
