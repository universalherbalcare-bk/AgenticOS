...
# NOTE (AgenticOS merge): this example previously included a "Local Python Agent"
# and a "Code Agent" built on PythonTools and E2BTools. Both toolkits were removed
# from the brain plane on security grounds - PythonTools executed code in-process
# with `safe_globals = globals()`, and hosted sandboxes moved execution off the
# machine whose policy we control. Code execution in AgenticOS belongs to the edge
# plane's container sandbox. The reasoning/team behaviour this example teaches is
# unchanged. See docs/DELETION-LEDGER.md.
